...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const generatePreviewHTML = () => {
        ...
        const styles = `
            <style>
                ...
                #preview-content { --accent: ${accent}; --text: #0f172a; --name-color: ${nameColor}; --muted: #64748b; color: var(--text); background: white; line-height: 1.6; }
                #preview-content * { margin:0; padding:0; box-sizing: border-box; font-family: 'Outfit', sans-serif; }
                
                #preview-content [contenteditable="true"]:hover { outline: 1px dashed var(--accent); cursor: text; }
                #preview-content [contenteditable="true"]:focus { outline: 2px solid var(--accent); background: #f8fafc; }
                ...
                #preview-content .nav { 
                    ...
                    position: sticky; 
                    top: 0; 
                    background: ${state.headerBgImg ? `linear-gradient(rgba(255,255,255,${state.headerOpacity / 100}), rgba(255,255,255,${state.headerOpacity / 100})), url('${state.headerBgImg}')` : 'white'};
                    backdrop-filter: blur(8px);
                    -webkit-backdrop-filter: blur(8px);
                    background-size: ${state.bgWidth}% ${state.bgHeight}%;
                    background-position: center;
                    background-repeat: no-repeat;
                    z-index: 1000;
                    box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);
                }
                #preview-content .nav-top {
                    display: flex;
                    justify-content: space-between;
                    align-items: center;
                    padding: 1.25rem 5% 0.5rem 5%;
                    width: 100%;
                }
                #preview-content .nav-bottom-bar {
                    display: flex;
                    justify-content: center;
                    align-items: center;
                    padding: 0.5rem 5% 1.75rem 5%;
                ...
                #preview-content .brand-name-straight { 
                    ...
                    text-shadow: 0 4px 15px rgba(0,0,0,0.06);
                    transition: transform 0.3s ease;
                }
                #preview-content .brand-name-straight:hover { transform: translateY(-2px); }
                
                #preview-content .nav-links { display:flex; list-style:none; gap:2rem; }
                #preview-content .nav-links a { text-decoration:none; color: var(--muted); font-weight:600; font-size:0.95rem; transition:0.3s; }
                #preview-content .nav-links a:hover { color: var(--accent); }
                
                #preview-content .hero { 
                    padding: 80px 5%; 
                    text-align:center; 
                    background: ${state.heroImg ? `linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url('${state.heroImg}')` : 'linear-gradient(180deg, #fafafa 0%, white 100%)'};
                ...
                #preview-content .hero h1 { font-size: 3.5rem; letter-spacing: -2px; margin-bottom:1.5rem; line-height: 1.1; color: var(--name-color) !important; }
                #preview-content .hero p { font-size: 1.2rem; color: ${state.heroImg ? 'rgba(255,255,255,0.9)' : 'var(--muted)'}; max-width: 600px; margin: 0 auto 2.5rem; }
                
                #preview-content section { scroll-margin-top: 80px; }
                
                #preview-content .services { padding: 80px 5%; }
                #preview-content .grid { display:grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 2rem; }
                ...
                #preview-content .card-img {
                    width: 100%;
                    height: 100%;
                    object-fit: contain; /* Full source visibility */
                    transform: scale(var(--img-scale, 1));
                    transition: transform 0.6s cubic-bezier(0.4, 0, 0.2, 1);
                }
                #preview-content .card:hover .card-img {
                    transform: scale(calc(var(--img-scale, 1) + 0.05));
                }
                ...
                #preview-content .card:hover { transform: translateY(-8px); box-shadow: 0 30px 60px -12px rgba(0,0,0,0.12); border-color: var(--accent); }
                #preview-content .card h3 { margin-bottom: 1rem; font-size: 1.5rem; }
                #preview-content .card p { color: var(--muted); font-size: 0.95rem; }
                
                #preview-content .edit-hint { position: absolute; top: 10px; right: 10px; font-size: 0.7rem; color: var(--accent); opacity: 0; transition: 0.3s; pointer-events: none; }
                #preview-content .card:hover .edit-hint { opacity: 0.6; }
                
                #preview-content .footer { padding: 80px 5%; border-top: 1px solid #f1f5f9; text-align:center; background: #fafafa; }
                #preview-content .social { display:flex; justify-content:center; gap: 1.5rem; margin-bottom: 2rem; }
                #preview-content .social a { color: var(--name-color); font-size: 1.2rem; transition:0.3s; }
                #preview-content .social a:hover { transform: scale(1.2); filter: brightness(1.2); }

                /* Contact Card Styles for 3D Logo Integration */
                ...
                @media (max-width: 768px) {
                    ...
                    #preview-content .contact-card { padding: 1.2rem; }
                }
            </style>
        `;
        ...
        const socialHTML = `
            <div class="social" style="display: flex; gap: 1.5rem; flex-wrap: wrap; justify-content: center; margin-top: 2rem;">
                ...
                .map(([key, val]) => {
                    ...
                    return `
                        <a href="${finalUrl}" target="_blank" class="soc-link" data-id="${key}" title="${key}" style="display: flex; align-items: center; gap: 1rem; background: white; padding: 0.8rem 1.5rem; border-radius: 12px; border: 1px solid #e2e8f0; text-decoration: none; box-shadow: 0 4px 10px rgba(0,0,0,0.05); transition: 0.4s;">
                            <img src="${logoUrl}" alt="${key}" style="width: 32px; height: 32px; object-fit: contain;">
                            <span style="font-size: 1rem; color: var(--text); font-weight: 700; text-transform: capitalize;">${key}</span>
                        </a>`;
                }).join('')}
            </div>
        `;
        ...
        return `
            ...
            <section class="products" id="products" style="padding: 80px 5%; background: white;">
                <div style="max-width: 1200px; margin: 0 auto;">
                    <h2 style="font-size: 2.5rem; text-align: center; margin-bottom: 3rem; color: var(--text);">Our Products</h2>
                    <div class="grid">
                        ${services.map((s, idx) => `
                            <div class="card" data-index="${idx}">
                                <div class="card-img-container">
            ...
            <footer class="footer" id="contact">
                ...
                ${socialHTML}
                <p style="color:var(--muted); font-size:0.85rem; margin-top: 3rem;">SiteBuilder AI v10.3 | &copy; 2026 <span class="brand-name" style="color: ${nameColor};">${name}</span>.</p>
            </footer>` : ''}
        `;
    };
    ...
    elements.viewportBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            elements.previewContainer.style.width = btn.dataset.width;
        });
    });
